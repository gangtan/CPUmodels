...
Module X86_Decode.
  ...
  Definition iload_op80 (seg:segment_register) (op:operand) : Conv (pseudo_reg size80) :=
    match op with 
      ...
      | FPS_op fm => load_fpu_reg (fpu_from_int fm (ps_reg size3 0)) 
  ...
  Definition iload_op32 (seg:segment_register) (op:operand) : Conv (pseudo_reg size32) :=
    match op with 
      ...
      | FPS_op fm => tmp <- load_fpu_reg (fpu_from_int fm (ps_reg size3 0)); 
                     cast_u size32 tmp
  ...
  Definition iset_op80 (seg:segment_register) (p:pseudo_reg size80) (op:operand) :
    ...
    match op with 
      ...
      | Reg_op r => tmp <- cast_u size32 p;
                    set_reg tmp r
      | FPS_op fm => set_fpu_reg p (fpu_from_int fm (ps_reg size3 0))
  ...
  Definition iset_op32 (seg:segment_register) (p:pseudo_reg size32) (op:operand) :
    ...
    match op with 
      ...
      | FPS_op fm => tmp <- cast_u size80 p;
                     set_fpu_reg tmp (fpu_from_int fm (ps_reg size3 0))
...
(*
    ...
    Things to check : -Loading from memory may not be handled correctly (load_mem32 is used in all cases, then converted to bin80)
                      -Do segment registers come into play at all in x87 floating-point architecture
...
(*Start of floating-point conversion functions *)
  ...
  *)
  ...
  Definition conv_FST (op: operand) := 
    topp <- get_stacktop;
    zero <- load_Z size3 0;
    ...
    | FPM_op a =>      (*Copy st(0) to memory *)
      ...
      set_mem32 SS psreg_val addr  (*Covers only 32-bit memory case *)
    | _ => emit error_rtl
    end.

  Definition conv_FSTP (op: operand) := 
    conv_FST op;;
    topp <- get_stacktop;
    empty <- load_Z size2 3;
  ...
  Definition conv_simple_arith (d : bool) (st_i : operand) 
     ...
     | FPM_op a =>    (*needs to be some mechanism to check fpm32, fpm64, fpm80. for now, fpm32 is handled *)
        addr <- compute_addr a; 
        val <- load_mem32 SS addr;
