...
Module X86_RTL := RTLWithFloat.RTL(X86_MACHINE).
...
Module X86_Decode.
  ...
  Definition iload_op80 (seg:segment_register) (op:operand) : Conv (pseudo_reg size80) :=
    match op with 
      | Imm_op i => tmp <- load_int i;
                           cast_u size80 tmp
      | Reg_op r => tmp <- load_reg r;
                           cast_u size80 tmp
      | Address_op a => p1 <- compute_addr a ; load_mem80 seg p1
      | FPS_op fm => load_fpu_reg fm 
      | FPM_op a => p1 <- compute_addr a ; load_mem80 seg p1
  ...
  Definition iload_op32 (seg:segment_register) (op:operand) : Conv (pseudo_reg size32) :=
    match op with 
      ...
      | FPS_op fm => tmp <- load_fpu_reg fm;
                            cast_u size32 tmp
      | FPM_op a => p1 <- compute_addr a ; load_mem32 seg p1
      | Offset_op off => p1 <- load_int off;
                          load_mem32 seg p1
    end.
  ...
  Definition iload_op16 (seg:segment_register) (op:operand) : Conv (pseudo_reg size16) :=
    match op with 
      ...
      | FPS_op fm => tmp <- load_fpu_reg fm;
                     cast_u size16 tmp
      | FPM_op a => p1 <- compute_addr a ; load_mem16 seg p1
  ...
  Definition iload_op8 (seg:segment_register) (op:operand) : Conv (pseudo_reg size8) :=
    match op with 
      ...
      | FPS_op fm => tmp <- load_fpu_reg fm;
                     cast_u size8 tmp
      | FPM_op a => p1 <- compute_addr a ; load_mem8 seg p1
...
(* All memory is going to be 32-bit, even floating-point. Only floating-point stack is 80-bit.
 ...
 Definition set_mem p w (seg:segment_register) : pseudo_reg (opsize (op_override p) w) ->
  ...
  Definition iset_op80 (seg:segment_register) (p:pseudo_reg size80) (op:operand) :
    ...
    match op with 
      ...
      | Reg_op r => tmp <- cast_u size32 p;
                    set_reg tmp r
      | FPS_op fm => set_fpu_reg p fm
      | FPM_op r => addr <- compute_addr r ; tmp <- cast_u size32 p;
  ...
  Definition iset_op32 (seg:segment_register) (p:pseudo_reg size32) (op:operand) :
    ...
    match op with 
      ...
      | FPS_op fm => tmp <- cast_u size80 p;
                     set_fpu_reg tmp fm
      | FPM_op r => addr <- compute_addr r ; set_mem32 seg p addr
  ...
  Definition iset_op16 (seg:segment_register) (p:pseudo_reg size16) (op:operand) :
    ...
    match op with 
      ...
      | FPS_op fm => tmp <- cast_u size80 p;
                     set_fpu_reg tmp fm
      | FPM_op r => addr <- compute_addr r ; set_mem16 seg p addr
  ...
  Definition iset_op8 (seg:segment_register) (p:pseudo_reg size8) (op:operand) :
    ...
    match op with 
      ...
      |  FPS_op fm => tmp <- cast_u size80 p;
                      set_fpu_reg tmp fm
      | FPM_op r => addr <- compute_addr r ; set_mem8 seg p addr
...
*)
  ...
  Definition conv_BT (param1: bool) (param2: bool)
    ...
    match op1 with
      ...
      | FPS_op f1 => emit error_rtl
      | FPM_op fp =>
        psaddr <- compute_addr fp;
        btmem psaddr
      | Address_op a => 
...
(*
    ...
    Things to check : -Loading from memory may not be handled correctly (load_mem80 is used in all cases)
...
Section X86FloatSemantics.
  ...
  (*Floating-point constants to represent: +1.0, +0.0, pi, lo`, log_e_2, log_2_10, log_10_2
    Based on section 5.2.5 of manual. Use table 4-5 for reference.
    Also, there must be support of 7 different data types: 
    single precision, double precision, double extended precision, signed word integer, signed doubleword integer,
    signed quadword integer, and packed BCD decimal integers. 
*)

   (*
    Fixpoint binToBvector (n:nat) (f:Z -> bool) : Bvector n :=
     match n return Bvector n with 
     | O => Bnil
     | S m => 
       let f' := (fun i:Z => f (i+1)%Z) in 
       let bv := binToBvector m f' in
       Bcons (f 0%Z) m bv
     end.

   (*Pad the front of bv (after most-significant bit) with n zeroes *)
   Definition pad_with_zeroes (n m:nat) (bv: Bvector m) : Bvector (n + m):= Vextend bool n m (Bvect_false n) bv.


Fixpoint binToBvector1 (n:nat) (f:Z -> bool) : Bvector n :=
  match n return Bvector n with 
  | O => Bnil
  | S m => 
    let f' := (fun i:Z => f (i-1)%Z) in 
    let bv := binToBvector1 m f' in
      Bcons (f m%Z) m bv
  end.

Fixpoint string_to_Z_bool1 (s:string) : Z -> bool :=
  match s with 
  | EmptyString => (fun i: Z => false)
  | String a s0 =>
     let b := if ascii_dec a "0"%char then false else true in
     (fun i: Z => if zeq i (length s) then b else string_to_Z_bool s0 (i + 1))
  end.
*)

(*
   Definition intToBvector n (i:Word.int n) : Bvector n := 
    let f := Word.bits_of_Z n (Word.unsigned i) in 
      binToBvector n f.

   Definition string_to_bvector n (s:string) : Bvector n := binToBvector n (string_to_Z_bool s).


Fixpoint rev_vect n (l:Bvector n) : Bvector n :=
    match l return Bvector n with
      | Bnil => Bnil
      | Bcons _ l' => rev_vect l' ++ bool :: Bnil
    end.
*)
...
(*Start of floating-point conversion functions *)
  ...
  Definition Z_to_fpu_register0(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rzero
    | 1%Z => Rone
  ...
  Definition Z_to_fpu_register1(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rone
    | 1%Z => Rtwo
  ...
  Definition Z_to_fpu_register2(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rtwo
    | 1%Z => Rthree
  ...
  Definition Z_to_fpu_register3(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rthree
    | 1%Z => Rfour
  ...
  Definition Z_to_fpu_register4(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rfour
    | 1%Z => Rfive
  ...
  Definition Z_to_fpu_register5(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rfive
    | 1%Z => Rsix
  ...
  Definition Z_to_fpu_register6(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rsix
    | 1%Z => Rseven
  ...
  Definition Z_to_fpu_register7(num: pseudo_reg size3) : fpu_register :=
   let (n) := num in
   match n with
    | 0%Z => Rseven
    | 1%Z => Rzero
 ...
 (*load val into an fpu-register stack where the top reg is stacktop and num is the number of registers from the top 
  ...
  Definition load_into_stack_i (val : pseudo_reg size80) (stacktop num : pseudo_reg size3): Conv unit:= 
     zero <- load_Z size3 0;
     one <- load_Z size3 1;
     two <- load_Z size3 2;
     ...
     t0 <- test eq_op stacktop zero;
     t1 <- test eq_op stacktop one;
     t2 <- test eq_op stacktop two;
     t3 <- test eq_op stacktop three;
     t4 <- test eq_op stacktop four;
     t5 <- test eq_op stacktop five;
     t6 <- test eq_op stacktop six;
     t7 <- test eq_op stacktop seven;
     
     emit if_rtl t0 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register0 num)));;
     emit if_rtl t1 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register1 num)));;
     emit if_rtl t2 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register2 num)));;
     emit if_rtl t3 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register3 num)));;
     emit if_rtl t4 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register4 num)));;
     emit if_rtl t5 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register5 num)));;
     emit if_rtl t6 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register6 num)));;
     emit if_rtl t7 (set_loc_rtl val (fpu_reg_loc (Z_to_fpu_register7 num))).  

 (*return the val of an fpu register where the top reg is stacktop and num is the number of registers from the top 
    where it is located, i.e num is i in st(i). *) 
  Definition set_from_stack_i (stacktop num: pseudo_reg size3) : Conv (pseudo_reg size80) := 

     let (stopp) := stacktop in
     let r0_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register0 num)) in
     let r1_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register1 num)) in
     let r2_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register2 num)) in
     let r3_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register3 num)) in
     let r4_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register4 num)) in
     let r5_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register5 num)) in
     let r6_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register6 num)) in
     let r7_val := get_loc_rtl (fpu_reg_loc (Z_to_fpu_register7 num)) in
  
     if (Zeq_bool stopp 0) then 
             fresh (fun ps: pseudo_reg size80 =>(r0_val ps))
     else if (Zeq_bool stopp 1) then
             fresh (fun ps: pseudo_reg size80 =>(r1_val ps))
     else if (Zeq_bool stopp 2) then
             fresh (fun ps: pseudo_reg size80 =>(r2_val ps))
     else if (Zeq_bool stopp 3) then
             fresh (fun ps: pseudo_reg size80 =>(r3_val ps))
     else if (Zeq_bool stopp 4) then
             fresh (fun ps: pseudo_reg size80 =>(r4_val ps))
     else if (Zeq_bool stopp 5) then
             fresh (fun ps: pseudo_reg size80 =>(r5_val ps))
     else if (Zeq_bool stopp 6) then
             fresh (fun ps: pseudo_reg size80 =>(r6_val ps))
     else
             fresh (fun ps: pseudo_reg size80 =>(r7_val ps)).
  ...
  Definition conv_FLD (op: operand) := 
     topp <- get_stacktop;
     zero <- load_Z size3 0;
     match op with
     | FPS_op reg => val <- load_fpu_reg reg;
       conv_FDECSTP;; 
       load_into_stack_i val topp zero
     | FPM_op a =>  
        addr <- compute_addr a; 
        val <- load_mem80 SS addr;
               conv_FDECSTP;;
        load_into_stack_i val topp zero
     | _ => emit error_rtl
     end.
  ...
  Definition conv_load_fpconstant (b: binary80) : Conv unit := 
     ...
     cast_decr <- cast_u size3 decrval;
     tagval <- load_Z size3 0;
     load_into_stack_i val decrval tagval;;
  ...
  Definition add_to_stacktop80 (sec : pseudo_reg size80) :=
     zero <- load_Z size3 0;
     topp <- get_stacktop;
     stacktop_val <- set_from_stack_i topp zero;
     ...
     let sum := b80_plus mode_NA first_val sec_val in
  ...
  Definition sub_from_stacktop80 (sec : pseudo_reg size80) := 
    zero <- load_Z size3 0;
     ...
     let sub := b80_minus mode_NA first_val sec_val in
  ...
  Definition mult_stacktop80 (sec : pseudo_reg size80) := 
     ...
     let mult := b80_mult mode_NA first_val sec_val in
  ...
  Definition div_stacktop80 (sec : pseudo_reg size80) :=
     ...
     let div := b80_div mode_NA first_val sec_val in
  ...
  Definition sqrt_stacktop80 := 
     ...
     let rt := b80_sqrt mode_NA first_val in
  ...
  Definition conv_simple_arith (d : bool) (st_i : operand) 
                               (operation : pseudo_reg size80 -> Conv (pseudo_reg size80)) := 
     topp <- get_stacktop;
     zero <- load_Z size3 0;
     match st_i with 
     | FPS_op r =>
        next_val <- load_fpu_reg r;
        curr_val <- operation next_val;
        
        match d with 
        | false => load_into_stack_i curr_val topp zero  (*load sum into stacktop *) 
        | true => set_fpu_reg curr_val r  (*load sum into st(i) *)
        end

     | FPM_op a =>    (*needs to be some mechanism to check fpm32, fpm64, fpm80 *)
        addr <- compute_addr a; 
        sti <- load_mem80 SS addr;
        curr_val <- operation sti;
        load_into_stack_i curr_val topp zero (*add from memory to stacktop *)
     | _ => emit error_rtl
     end.
  ...
  Definition conv_simple_arith_and_pop (st_i : option operand) (operation : pseudo_reg size80 -> Conv (pseudo_reg size80))
     : Conv unit := 
     toploc <- get_stacktop;
     empty <- load_Z size2 3;
     ...
     | None => (* perform operation on st(0) and st(1) and store in st(0) *)
         onee <- load_Z size3 1;
         st1 <- set_from_stack_i toploc onee;
         let (st1val) := st1 in
         val <- operation st1;
         zero <- load_Z size3 0;
         load_into_stack_i val toploc zero;;
         conv_FINCSTP
     | Some op => 
  ...
  Definition conv_FADD (d :bool) (st_i: operand) : Conv unit := conv_simple_arith d st_i add_to_stacktop80.
  Definition conv_FSUB (d : bool) (r1 r2: option bool) (st_i : operand) : Conv unit := conv_simple_arith d st_i sub_from_stacktop80.
  Definition conv_FMUL (d : bool) (st_i : operand) : Conv unit := conv_simple_arith d st_i mult_stacktop80.
  Definition conv_FDIV (d : bool) (r1 r2: option bool) (st_i : operand) : Conv unit := conv_simple_arith d st_i div_stacktop80.

  Definition conv_FADDP (st_i : option operand) : Conv unit := conv_simple_arith_and_pop st_i add_to_stacktop80.
  Definition conv_FSUBP (st_i : option operand) : Conv unit := conv_simple_arith_and_pop st_i sub_from_stacktop80.
  Definition conv_FMULP (st_i : option operand) : Conv unit := conv_simple_arith_and_pop st_i mult_stacktop80.
  Definition conv_FDIVP (st_i : option operand) : Conv unit := conv_simple_arith_and_pop st_i div_stacktop80.
(*  
  Definition conv_simple_integer_arith (st_i : operand) 
     ...
     (* let (d, b, i) := addr in  eventually going to have to figure out how to differentiate between size16 and size32 memory *)
       ...
       int_operation (ps_reg size80 (bits_of_b80 val))
     | _ => emit error_rtl
     end.
*)
...
Definition conv_FCOM (op1: option operand) := 
     topp <- get_stacktop;
     zero <- load_Z size3 0;
     onee <- load_Z size3 1;
     zer3 <- cast_u size3 zero;
     one3 <- cast_u size3 onee;
     st0 <- set_from_stack_i topp zer3;
     let (st0val) := st0 in
     let binst0 := b80_of_bits st0val in
     match op1 with 
     | None => (* Compare st(0) to st(1) *)
	 st1 <- set_from_stack_i topp one3;
         let (st1val) := st1 in
...
Definition conv_FCOMP (op1 : option operand) := 
    conv_FCOM op1;;
    toploc <- get_stacktop;
    empty <- load_Z size2 3;
    update_tag toploc empty;;
...
Fixpoint parse_instr_aux
  (n:nat) (loc:int32) (len:positive) (ps:DecodeWithFloat.X86_PARSER.instParserState) : 
  RTL ((prefix * instr) * positive) := 
  match n with 
    | 0%nat => Fail _ 
    | S m => b <- get_byte loc ; 
             match DecodeWithFloat.X86_PARSER.parse_byte ps b with 
               | (ps', nil) => 
...
Definition parse_instr (pc:int32) : RTL ((prefix * instr) * positive) :=
  ...
  let real_pc := Word.add seg_start pc in
    parse_instr_aux 15 real_pc 1 DecodeWithFloat.X86_PARSER.initial_parser_state.
