...
Definition non_cflow_instr (pfx:prefix) (ins:instr) : bool := 
  match ins with 
    ...
    *)
    ...
    | IMUL w op1 opopt iopt => only_seg_or_op pfx
    ...
    (* | CMP false op1 op2 => only_gs_seg_override pfx *)
    ...
    (* JGM:  The parser doesn't rule out other kinds of operands, so I'm
       generalizing this to allow for it.  Note, however, that the machine
       should trap if there isn't an address operand. 
    | LEA op1 (Address_op a) => only_gs_seg_override pfx && no_imm_op op1
    *)
    | LEA op1 op2 => only_gs_seg_override pfx && no_imm_op op1
