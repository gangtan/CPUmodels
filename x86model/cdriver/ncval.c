...
#include "/home/tristan/Code/NaClVerifier/x86model/cdriver/tables.h"
#include "/home/tristan/Code/NaClVerifier/x86model/cdriver/driver.h"
 ...
 */
#define NACL_MAX_INPUT_LINE 4194304//4096
...
int MyAnalyzeSegmentSections(ncfile *ncf, Bool *res) {
  int badsections = 0;
  int ii;
  const Elf_Phdr *phdr = ncf->pheaders;
  ...
  for (ii = 0; ii < ncf->phnum; ii++) {
    ...
    *res = (verifier(&ncflow,&dbranch,&ibranch,ncf->data + (phdr[ii].p_vaddr - ncf->vbase),phdr[ii].p_memsz)==0)?FALSE:TRUE;
  }
  return -badsections;
}
...
static Bool AnalyzeSegmentCodeSegments(ncfile *ncf, const char *fname) {
  ...
  NaClLog(LOG_INFO, "Validated %s\n", fname);
    ...
    if (MyAnalyzeSegmentSections(ncf,&result) < 0) {
      NaClLog(LOG_INFO, "%s: text validate failed\n", fname);
    }
    NaClReportSafety(result, fname);
    NaClLog(LOG_INFO, "Validated %s\n", fname);
...
static Bool NaClValidateAnalyzeBytes(NaClValidateBytes* data) {
  ...
  if (!dfa) {
    ...
    printf("DFA validator\n");
    // Assuming @ is not used in the hex text
    return_value = (1 == verifier(&ncflow,&dbranch,&ibranch,data->bytes,data->num_bytes)) ? TRUE : FALSE;
    NaClReportSafety(return_value, "");
  }
#endif
  return return_value;
...
static int GrokFlags(int argc, const char *argv[]) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    } else if (GrokBoolFlag("--dfa", arg, &dfa)) {
