...
Definition set_cat_re (s:RESet.t) (r:regexp): RESet.t := 
  match r with
    ...
    | Zero => RESet.empty
    | _ => RESetF.map (re_set_build_map (fun r1 => Cat r1 r)) s
  end.
...
Fixpoint prebase (r:regexp): RESet.t := 
  match r with
    | Eps | Zero => RESet.empty
    | Char _ | Any => RESet.singleton Eps
    | Cat r1 r2 => RESet.union ((prebase r1) $ r2) (prebase r2)
    | Alt r1 r2 => RESet.union (prebase r1) (prebase r2)
...
Lemma set_cat_re_intro1 : forall r s r2,
  RESet.In r s -> r2 = Eps -> RESet.In r (s $ r2).
Proof. crush. Qed.

Lemma set_cat_re_intro2 : forall r s r1 r2,
  RESet.In r1 s -> r = Cat r1 r2 -> r2 <> Eps -> r2 <> Zero
    -> RESet.In r (s $ r2).
Proof. destruct r2; 
...
Lemma set_cat_re_elim : forall r s r2,
  RESet.In r (s $ r2) -> 
    (r2=Eps /\ RESet.In r s) \/
    (r2=Zero /\ False) \/
    (r2<>Eps /\ r2<>Zero /\ exists r1, RESet.In r1 s /\ r = Cat r1 r2).
...
Fixpoint pdrv (a: char_t) (r:regexp) : RESet.t := 
  match r with
    | Zero | Eps => RESet.empty
    | Char b => 
      if char_eq_dec a b then RESet.singleton Eps
      else RESet.empty
    | Any => RESet.singleton Eps
    | Cat r1 r2 => 
      if nullable r1 then RESet.union ((pdrv a r1) $ r2) (pdrv a r2)
      else (pdrv a r1) $ r2
    | Alt r1 r2 => RESet.union (pdrv a r1) (pdrv a r2)
    | Star r1 => (pdrv a r1) $ (Star r1)
  end.
...
| InrEps : forall s, s = nil -> in_re Eps s
| InrChar : forall c s, s = c::nil -> in_re (Char c) s
| InrAny : forall c s, s = c::nil -> in_re Any s
| InrCat : forall a1 a2 s1 s2 s,
  in_re a1 s1 -> in_re a2 s2 -> s = s1 ++ s2 -> 
  in_re (Cat a1 a2) s
| InrAlt_l : forall a1 a2 s,
  in_re a1 s -> in_re (Alt a1 a2) s
| InrAlt_r : forall a1 a2 s,
  in_re a2 s -> in_re (Alt a1 a2) s
| InrStar_eps : forall a s, s = nil -> in_re (Star a) s
| InrStar_cons : forall a s1 s2 s,
  in_re a s1 -> in_re (Star a) s2 -> 
  s1 <> nil -> s = s1 ++ s2 -> in_re (Star a) s.
...
Section DFA.
  ...
  Proof. generalize build_table_prop. 
    ...
    Case "reject states correct".
      change false with (reset_always_rejects (RESet.singleton Eps)).
      rewrite map_nth.
      erewrite Coqlib.nth_error_some_nth by eassumption.
