...
Inductive fpu_register : Set := R7 | R6 | R5 | R4 | R3 | R2 | R1 | R0.
Definition fpu_register_eq_dec : 
  forall (x y:fpu_register), {x=y} + {x<>y}.
  intros ; decide equality.
Defined.

Definition Z_to_fpu_register(n:Z) :=
  match n with
    | 0 => R0
    | 1 => R1
    | 2 => R2
...
Inductive fp_status_register : Set := Busy | CC | Top | Es | Sf | Pe | Ue | Oe | Ze | De | Ie.
Definition fp_status_register_eq_dec : 
  forall (x y:fp_status_register), {x=y} + {x<>y}.
  intros ; decide equality.
Defined.

Inductive fp_control_register : Set := Reserved | IC | RC | PC | Pm | Um | Om | Zm | Dm | Im.
...
Inductive fpu_tagWords : Set := 
| Tag0 : fp_tagWord_register -> fpu_tagWords
| Tag1 : fp_tagWord_register -> fpu_tagWords
| Tag2 : fp_tagWord_register -> fpu_tagWords
| Tag3 : fp_tagWord_register -> fpu_tagWords
| Tag4 : fp_tagWord_register -> fpu_tagWords
| Tag5 : fp_tagWord_register -> fpu_tagWords
| Tag6 : fp_tagWord_register -> fpu_tagWords
| Tag7 : fp_tagWord_register -> fpu_tagWords.

Inductive fp_lastOperPtr_register : Set := 
| Valid : address -> fp_lastOperPtr_register
| Undefined.

Definition fp_address := address.

(* Easier for decoding and semantics purposes to have these two in operand below
Inductive fp_operand : Set :=
| FPS_op : fpu_register -> fp_operand (*Either top of stack i.e. st(0) or st(i) *)
| FPM_op : fp_address -> fp_operand. (*Same as int instruction address*)
*)
...
| FPS_op : fpu_register -> operand
| FPM_op : fp_address -> operand. 
...
| FADDP : forall (op1: operand), instr
| FBLD : forall (op1: operand), instr
| FBSTP : forall (op1: operand), instr
| FCHS : instr
| FCLEX : instr
| FCOM : forall (op1: operand), instr
| FCOMP : forall (op1: operand), instr
| FCOMPP : instr
| FCOMIP : forall (op1: operand), instr
| FCOS : instr
| FDECSTP : instr
(*In the stack case, this kind of instruction has a 2-bit R parameter, 
which I have broken up into r1 and r2. Details are shown in table B-38 of the manual *)
| FDIV : forall (d: bool) (r1: option bool) (r2: option bool) (op1: operand), instr
| FDIVP : forall (op1: operand), instr
| FDIVR : forall (d: bool) (r1: option bool) (r2: option bool) (op1: operand), instr
...
| FMULP : forall (op1: operand), instr
| FNOP : instr
| FPATAN : instr
| FPREM : instr
| FPREM1 : instr
...
| FSUB : forall (d: bool) (r1: option bool) (r2: option bool) (op1: operand), instr
| FSUBP : forall (op1: operand), instr
| FSUBR : forall (d: bool) (r1: option bool) (r2: option bool) (op1: operand), instr
